...
/* ! Medoo 0.9.7 - Copyright 2014, Angel Lai - MIT license - http://medoo.in */
            ...
            if (isset($match[3])) {
                ...
                switch (gettype($value)) {
                    case 'NULL':$fields[] = $column . ' = NULL';
                        break;
                    case 'array':preg_match("/\(JSON\)\s*([\w]+)/i", $key, $column_match);
                        $fields[] = isset($column_match[0]) ? $this->column_quote($column_match[1]) . ' = ' . $this->quote(json_encode($value)) : $column . ' = ' . $this->quote(serialize($value));
                        break;
